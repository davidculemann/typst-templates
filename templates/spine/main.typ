// Spine CV, a Typst resume template.
//
...
// Edit in a browser: https://www.jobsprout.ai/resume-templates/spine
...
// ONE tasteful block: a thin full-height bar down the left page edge (painted by the
// page background so it spans every page). Otherwise ink on white.
#let ink = rgb("#1f2430")
...
// Document shell: set-rules + PDF metadata + the left spine bar. The bar sits in the
// (widened) left margin so body text clears it, and the page background paints it on
// every page. Inter is the single family (standard rule 1); regular + semibold + bold.
#let spine-cv(
  author: "",
  font: "Inter",
  font-size: 10.5pt,
  ...
  set page(
    paper: paper,
    margin: (left: margin + 18pt, top: margin, bottom: margin, right: margin),
    background: place(top + left, rect(width: BAR-WIDTH, height: 100%, fill: rgb(accent-color))),
  )
...
#let masthead(author: "", profession: "", contact: "", accent-color: "#2f3e5c") = {
  ...
  if has-name or has-prof or has-contact {
    block(above: 0pt, below: 1.1em, breakable: false, {
      ...
      // No masthead rule: the full-height accent bar down the left edge already gives
      // the page its structure, so a divider here would only add clutter.
    })
  }
}

// Section label: accent UPPERCASE + tracking over a thin accent rule. Sticky.
#let cv-section(title, accent-color: "#2f3e5c") = {
...
#let spine-entry(title: "", subtitle: "", dates: "", location: "", meta: "", body) = {
...
#let spine-language(language: "", level: "") = {
  grid(
    columns: (1fr, auto),
...
#masthead(author: "Jane Smith", profession: "Director of Product", contact: { link("mailto:jane.smith@example.com")[jane.smith\@example.com] + h(0.5em) + sym.bar.v + h(0.5em) + [+1 (415) 555 0148] + h(0.5em) + sym.bar.v + h(0.5em) + [San Francisco, CA] }, accent-color: "#2f3e5c")

#cv-section("Summary", accent-color: "#2f3e5c")
...
#cv-section("Education", accent-color: "#2f3e5c")
...
#cv-section("Experience", accent-color: "#2f3e5c")
...
#cv-section("Projects", accent-color: "#2f3e5c")
...
#cv-section("Certifications", accent-color: "#2f3e5c")
...
#cv-section("Awards", accent-color: "#2f3e5c")
