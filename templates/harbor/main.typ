// Harbor CV, a Typst resume template.
//
...
// Edit in a browser: https://www.jobsprout.ai/resume-templates/harbor
...
// used as ONE tasteful block: a full-height right sidebar (painted by the page
...
// paints the accent panel down the right PANEL-FRAC of EVERY page (so a long CV keeps
...
// columns are composed by wrapSections through #cv-layout. Nunito Sans is the single
...
#let harbor-cv(
  ...
  set page(
    ...
    background: place(top + right, rect(width: PANEL-FRAC, height: 100%, fill: rgb(accent-color))),
  )
...
// Two-column layout as a BREAKABLE grid (aside RIGHT), so a long aside flows onto
// page 2+ alongside the main column. It was previously `place`d at the top,
// contributing zero flow height: that let the main column alone drive pagination,
// but an aside taller than the page then kept drawing past the paper edge,
// overlapping its own entries and losing text (a 45-language sidebar reached
// 2040pt on an 842pt page). The panel fill spans every page via the page
// background, so the rail still looks continuous when the aside runs short.
...
#let cv-layout(author: "", main: [], aside: [], hpad: 1.4cm, gap: 22pt) = {
  grid(
    columns: (1fr, PANEL-FRAC),
    column-gutter: gap,
    align: (left + top, left + top),
    block(inset: (left: hpad), {
      set text(fill: ink)
      main
    }),
    block(inset: (right: hpad, left: 12pt), {
...
// near-white) over the profession (muted white). photo is a VFS path string;
// none renders nothing and reserves no space.
...
#let harbor-rail-cred(name: "", issuer: "", date: "") = {
...
#let harbor-language(language: "", level: "") = {
  block(breakable: false, {
    set block(spacing: 0pt)
    set par(spacing: 0pt, justify: false)
    set text(hyphenate: false)
...
#let harbor-entry(title: "", subtitle: "", dates: "", location: "", meta: "", body) = {
  block(breakable: false, {
    set block(spacing: 0pt)
    set par(spacing: 0pt)
