...
- [x] selected [a name](https://github.com/typst/packages/blob/main/docs/manifest.md#naming-rules) that isn't the most obvious or canonical name for what the package does
  - Explanation:
    A vertex is the point where lines meet, which suits a centred layout that pairs its short sections two-up around a single axis.

    Following the guidance for template packages, the name is a unique, non-descriptive part (`vertex`) followed by a descriptive part (`cv`). It is not affiliated with any organisation, so no entity abbreviation applies.
...
- `typst init @preview/vertex-cv:0.1.0` into an empty directory, then `typst compile`, gives no diagnostics.
