...
- [x] selected [a name](https://github.com/typst/packages/blob/main/docs/manifest.md#naming-rules) that isn't the most obvious or canonical name for what the package does
  - Explanation:
    An index is a numbered finding aid. The design numbers its sections and sets the figures in the left margin as their own column, borrowing from Swiss editorial layout.

    Following the guidance for template packages, the name is a unique, non-descriptive part (`index`) followed by a descriptive part (`cv`). It is not affiliated with any organisation, so no entity abbreviation applies.
...
- `typst init @preview/index-cv:0.1.0` into an empty directory, then `typst compile`, gives no diagnostics.
