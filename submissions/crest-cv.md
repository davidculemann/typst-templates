...
- [x] selected [a name](https://github.com/typst/packages/blob/main/docs/manifest.md#naming-rules) that isn't the most obvious or canonical name for what the package does
  - Explanation:
    A crest is a heraldic device, and the letterpress associations are the point: this is the most ceremonial layout in the set, drawn for chambers and professional institutes.

    Following the guidance for template packages, the name is a unique, non-descriptive part (`crest`) followed by a descriptive part (`cv`). It is not affiliated with any organisation, so no entity abbreviation applies.
...
- `typst init @preview/crest-cv:0.1.0` into an empty directory, then `typst compile`, gives no diagnostics.
