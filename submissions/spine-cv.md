...
- [x] selected [a name](https://github.com/typst/packages/blob/main/docs/manifest.md#naming-rules) that isn't the most obvious or canonical name for what the package does
  - Explanation:
    A spine is the bound edge of a book. The design runs a narrow band of colour down the left edge of every page, which is what holds a two-page CV together as one document.

    Following the guidance for template packages, the name is a unique, non-descriptive part (`spine`) followed by a descriptive part (`cv`). It is not affiliated with any organisation, so no entity abbreviation applies.
...
- `typst init @preview/spine-cv:0.1.0` into an empty directory, then `typst compile`, gives no diagnostics.
