...
- [x] selected [a name](https://github.com/typst/packages/blob/main/docs/manifest.md#naming-rules) that isn't the most obvious or canonical name for what the package does
  - Explanation:
    Quietude is stillness. The design's only real feature is the white space around it, at a 3cm margin. `minimal-cv` is the canonical name and is already taken upstream, so this is both non-canonical and unambiguous.

    Following the guidance for template packages, the name is a unique, non-descriptive part (`quietude`) followed by a descriptive part (`cv`). It is not affiliated with any organisation, so no entity abbreviation applies.
...
- `typst init @preview/quietude-cv:0.1.0` into an empty directory, then `typst compile`, gives no diagnostics.
