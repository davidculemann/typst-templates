// Codex CV, a Typst resume template.
//
...
// Edit in a browser: https://www.jobsprout.ai/resume-templates/codex
...
// masthead rule and the section labels; ink on white otherwise. Only one rule is ever
// drawn, under the masthead.
#let ink = rgb("#1f2430")
...
// Document shell + masthead. The signature is JUSTIFICATION: prose and bullets are set
// justified with hyphenation on, so every paragraph closes on a clean right edge and
// the page reads like a printed book rather than a form. New Computer Modern is the
// single family (standard rule 1); it ships no SemiBold, so weights are regular +
// bold only.
#let codex-cv(
  author: "",
  profession: "",
  ...
  // hyphenate: true is not decoration here - justified text without it opens rivers.
  set text(font: font, size: font-size, fill: ink, lang: lang, ligatures: true, hyphenate: true)
  set page(margin: margin, paper: paper)
  set par(justify: true, leading: leading)

  show link: set text(fill: muted)
  // A solidus between two letters is a Typst break opportunity, so "A/B testing" and
  // "CI/CD" split across lines. In a template whose whole premise is justified
  // typographic craft that is not acceptable; box the pair so it stays whole.
  show regex("\\p{L}/\\p{L}"): it => box(it)

  // Masthead: name, role and contact flush left over a single accent rule. Headings
  // never justify (only prose does), so the masthead is set ragged.
...
// Section label: accent UPPERCASE + tracking, no rule (the justified block itself is
// the structure). `below: 0.9em` gives the first entry natural air.
#let cv-section(title, accent-color: "#3b3a4d", gap: 1.7em) = {
  block(above: gap, below: 1em, sticky: true, {
    set block(spacing: 0pt)
    set par(spacing: 0pt, justify: false)
    set text(hyphenate: false)
    text(size: label-size, weight: "bold", tracking: 1.1pt, fill: rgb(accent-color), upper(title))
  })
}

// Entry: the heading line stays ragged and unhyphenated (a justified two-word heading
// would space out grotesquely); only the body below is justified.
#let codex-entry(title: "", subtitle: "", dates: "", location: "", meta: "", body) = {
...
// Language row: language left, level muted right. Never justified.
#let codex-language(language: "", level: "") = {
  set par(justify: false)
  grid(
    columns: (1fr, auto),
...
#cv-section("Summary", accent-color: "#3b3a4d", gap: 1.74em)
...
#cv-section("Education", accent-color: "#3b3a4d", gap: 1.74em)
...
#cv-section("Experience", accent-color: "#3b3a4d", gap: 1.74em)
...
#cv-section("Projects", accent-color: "#3b3a4d", gap: 1.74em)
...
#cv-section("Skills", accent-color: "#3b3a4d", gap: 1.74em)

Product Strategy, Roadmapping, Data Analysis, SQL, A/B Testing, User Research

#cv-section("Languages", accent-color: "#3b3a4d", gap: 1.74em)
...
#cv-section("Certifications", accent-color: "#3b3a4d", gap: 1.74em)
...
#cv-section("Awards", accent-color: "#3b3a4d", gap: 1.74em)
